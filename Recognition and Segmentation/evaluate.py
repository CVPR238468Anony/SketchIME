...
def run_eval(opt=None, model=None, loader=None, dataset='test', write_result=False):
    ...
    if model is None:
        class_component_matrix_path=os.path.join('data', opt.dataset,'train',"class_component_matrix.npy")
        class_component_matrix=torch.tensor(np.load(class_component_matrix_path)).cuda()
        model = SketchModel(opt,class_component_matrix)
    if loader is None:
    ...
    right_recog_num=0
    for index,sketch1 in enumerate(testData):
        
        if sketch1["recog_label"]==recog_predictlist[index]:
            right_recog_num+=1
