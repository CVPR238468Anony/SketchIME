...
class SketchData(Data):
    def __init__(self, stroke_idx=None, x=None, edge_index=None, edge_attr=None, y=None,recog_label=None,pos=None, norm=None, face=None, **kwargs):
        # super(SketchData, self).__init__(x, edge_index, edge_attr, y, pos, norm, face, **kwargs)
        super(SketchData, self).__init__(x, edge_index, edge_attr, y, pos)
        self.recog_label=recog_label
    ...
    def __inc__(self, key,value,something):
...
class SketchDataset(torch.utils.data.Dataset):
    def __init__(self, opt, root, class_name, split='train', permutation=False):
        ...
        self.png_dir=osp.join(root,"pngs")
        self.component_dir=osp.join(root,"components")
