...
#plot_loss_acc(source=args.source, target=args.target, no_epochs=args.no_epochs) #args.source="amazon",args.target="webcam",args.no_epoches="10"
#plat_loss(amazon,webcam,100)
def plot_loss_acc(source, target, no_epochs):
    plt.rcParams.update({'font.size': 22})

    # specify path where in log folder where training logs are saved
    #/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/DeepCORAL/logs/amazon_to_webcam/10_epochs_128_s_128_t_batch_size
    pkldir = os.path.join("/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/DeepCORAL/logs", source + "_to_" + target,str(no_epochs) + "_epochs_128_s_128_t_batch_size") 
    #pkldir="/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/DeepCORAL/logs/amazon_to_webcam/10_epochs_128_s_128_t_batch_size"

    if Path(pkldir).is_dir():
        print("directory with pkl files is:", pkldir) #pkldir='logs/amazon_to_dlsr/10_epochs_128_s_128_t_batch_size'

    else:
        #print(Path(pkldir).is_dir())
    ...
    plt.plot(adaptation['target_accuracy'], label="test acc. w/ coral loss", marker='*', markersize=10)
    plt.plot(no_adaptation['target_accuracy'], label="test acc. w/o coral loss", marker='.', markersize=10)

    plt.plot(adaptation['source_accuracy'], label="training acc. w/ coral loss", marker='^', markersize=10)
    plt.plot(no_adaptation['source_accuracy'], label="training acc. w/o coral loss", marker='+', markersize=10)
    ...
    plt.show()
    fig.suptitle(source + "_to_" + target)
    #对图片进行保存/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/DeepCORAL/logs/amazon_to_webcam/100_epochs_128_s_128_t_batch_size/amazon_to_webcam_train_losses.jpg
...
def main():
    ...
    #plat_loss(amazon,webcam,100)
    plot_loss_acc(source=args.source, target=args.target, no_epochs=args.no_epochs) #args.source="amazon",args.target="dlsr",args.no_epoches="100"
