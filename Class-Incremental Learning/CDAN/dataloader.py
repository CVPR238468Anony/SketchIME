...
def get_sketch_dataloader(name_dataset, batch_size, train=True):
    ...
    mean_std[name_dataset]=get_mean_std_dataset(root_dir)  #添加
    ...
    data_transforms = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=mean_std[name_dataset]["mean"],std=mean_std[name_dataset]["std"])
        ])
...
#get_office_dataloader('amazon',batch_size=128)
def get_office_dataloader(name_dataset, batch_size, train=True):
    ...
    # root dir (local pc or colab)
    root_dir = "/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/images/datas/%s" % name_dataset 
    #root_dir = "/home/lihao/Deep-Unsupervised-Domain-Adaptation-master/images/datas/%s/images" % name_dataset #name_dataset=amazon
    ...
    __datasets__=['source8-15','person30','person40','person50','person2-5']
    ...
    # so our model won’t have to deal with the entire dataset every time.
    # shuffle data when training
    dataset_loader = DataLoader(dataset,batch_size=batch_size,shuffle=train,num_workers=0,drop_last=True)
