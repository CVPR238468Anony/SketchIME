from models.base.base import Trainer as Trainer
...
class FSCILTrainer(Trainer):
    ...
    def get_base_dataloader_meta(self):
        ...
        if self.args.dataset == 'sketch':
            train_txt_path = "data/index_list/" + self.args.dataset + "/session_" + str(0) + '.txt'
            test_txt_path = "data/index_list/" + self.args.dataset + "/test_" + str(0) + '.txt'
            trainset = self.args.Dataset.SketchPngDataset(root=self.args.dataroot, list_txt_url=train_txt_path)
            testset = self.args.Dataset.SketchPngDataset(root=self.args.dataroot, list_txt_url=test_txt_path)
        ...
        testloader = DataLoader(dataset=testset, batch_size=self.args.test_batch_size, shuffle=False, num_workers=0, pin_memory=True)
    ...
    def get_new_dataloader(self, session):
        ...
        if self.args.dataset == 'sketch':
            train_txt_path = "data/index_list/" + self.args.dataset + "/session_" + str(session) + '.txt'
            trainset = self.args.Dataset.SketchPngDataset(root=self.args.dataroot, list_txt_url=train_txt_path)
        ...
        if self.args.batch_size_new == 0:
            ...
            trainloader = DataLoader(dataset=trainset, batch_size=self.args.batch_size_new,
                                                      shuffle=True,
                                                      num_workers=0, pin_memory=True)
        ...
        if self.args.dataset == 'sketch':
            test_txt_path = "data/index_list/" + self.args.dataset + "/test_" + str(session) + '.txt'
            testset = self.args.Dataset.SketchPngDataset(root=self.args.dataroot, list_txt_url=test_txt_path)
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                print('test classes for this session:\n', np.unique(testloader.dataset.targets))
    ...
    def base_train(self, model, train_fsl_loader,train_gfsl_loader, optimizer, scheduler, epoch, args):
        tl = Averager()
        ta = Averager()
        # tqdm_gen = tqdm(zip(train_fsl_loader, train_gfsl_loader))
        tqdm_gen = tzip(train_fsl_loader, train_gfsl_loader)

        for _, batch in enumerate(tqdm_gen):
    ...
    def set_save_path(self):
        ...
        self.args.save_path = os.path.join(self.args.save_path, now_time)
