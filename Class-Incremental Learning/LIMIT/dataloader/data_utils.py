...
def set_up_datasets(args):
    if args.dataset == 'sketch':
        import dataloader.sketch.sketch as Dataset
        args.base_class = 118       # 118个基类
        args.num_classes = 374      # 总共374类
        args.way = 16               # 每次增量16个类
        args.shot = 5               # 每个类别5个样本
        args.sessions = 17          # 一共17个session
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'sketch':
        train_txt_path = "data/index_list/" + args.dataset + "/session_" + str(0) + '.txt'
        test_txt_path = "data/index_list/" + args.dataset + "/test_" + str(0) + '.txt'
        trainset = args.Dataset.SketchPngDataset(root=args.dataroot, list_txt_url=train_txt_path)
        testset = args.Dataset.SketchPngDataset(root=args.dataroot, list_txt_url=test_txt_path)
...
def get_new_dataloader(args,session):
    ...
    if args.dataset == 'sketch':
        train_txt_path = "data/index_list/" + args.dataset + "/session_" + str(session) + '.txt'
        trainset = args.Dataset.SketchPngDataset(root=args.dataroot, list_txt_url=train_txt_path)
    ...
    if args.dataset == 'sketch':
        test_txt_path = "data/index_list/" + args.dataset + "/test_" + str(session) + '.txt'
        testset = args.Dataset.SketchPngDataset(root=args.dataroot, list_txt_url=test_txt_path)
