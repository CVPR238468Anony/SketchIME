...
def train(model, source_loader, target_loader,optimizer, epoch, lambda_factor, cuda=False, supervised=False):
    ...
    for batch_idx in tnrange(train_steps):
        ...
        feature1=model.bottleneck(model.sharedNetwork(source_data))
        output1=model.fc(feature1)
        feature2=model.bottleneck(model.sharedNetwork(target_data))
        output2=model.fc(feature2)
        ...
        ddc_loss=DDC_loss(feature1,feature2)    

        # compute total loss (equation 6 paper)
        total_loss = classification_loss + lambda_factor*ddc_loss*0.0001  
